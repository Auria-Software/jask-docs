...
<Callout type="warning">
  ...
  `JaskSSO` depuis la console `source de données` dans Jask. **Jask ne passera
  jamais par `JaskAdmin`** pour accéder aux sites SharePoint. Cette étape est
  **obligatoire** pour pouvoir **donner accès à Jask** d'accéder à des sites
...
Séléctionnez `Connexions existantes` pour utiliser les secrets enregistrés dans Jask.
Séléctionnez le secret `...Admin` pour pouvoir rechercher l'entièreté des sites SharePoint de votre organisation car les droits le permettent.
![connectivité](@public/sharepoint/connection.png)

Rechercher le site SharePoint que vous voulez connecter à Jask à l'aide de la barre de recherche.
...
Séléctionnez le type de ressource `Drive`, `Liste` ou `Page ASPX` puis séléctionnez la ressource en question et testez la connexion.
...
Si vous souhaitez utiliser rapidement la source de données, vous pouvez lancer l'indexation manuellement. Cliquez sur les trois points à droite de la source de données puis sur le bouton numéro 2 sur le screenshot ci-dessous pour `Scanner`. Raffraichissez le tableau avec le bouton numéro 3 si le scan prend trop de temps. Enfin cliquez sur le bouton numéro 4 `Indexer` pour lancer l'indexation.
